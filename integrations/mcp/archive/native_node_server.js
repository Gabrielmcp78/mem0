...
/**
 * Gabriel's FoundationModels Memory MCP Server (Native Node.js)
 * 
 * Full native Node.js implementation with FoundationModels integration
...
class AppleIntelligenceMemoryServer {
  ...
  async setupConnections() {
    const candidates = [];
    if (this.qdrantUrl) candidates.push(this.qdrantUrl);
    // Add common fallbacks (host mode and docker-compose.simple mapped port)
    const envUrl = process.env.QDRANT_URL;
    if (envUrl && !candidates.includes(envUrl)) candidates.push(envUrl);
    ['http://localhost:6333', 'http://localhost:10333'].forEach(u => {
    ...
    for (const url of candidates) {
      try {
        ...
        console.error(`✅ Qdrant connection established at ${url}`);
        return;
      } catch (error) {
        console.error(`⚠️ Qdrant probe failed at ${url}:`, error.message);
        this.qdrantClient = null;
      }
    }

    console.error('❌ Qdrant connection failed for all candidates, continuing without vector store');
  }
  ...
  async ensureCollection() {
    try {
      ...
      if (!exists) {
        // Create collection with FoundationModels embedding dimensions
        await this.qdrantClient.createCollection(this.collectionName, {
          vectors: {
            size: 1536, // FoundationModels embedding size
            distance: 'Cosine',
          },
        });
  ...
  // FoundationModels Integration - Real Foundation Models on macOS 26
  async processWithAppleIntelligence(text, operation = 'embed') {
    try {
      ...
      throw new Error(`Unsupported FoundationModels operation: ${operation}`);
    } catch (error) {
      console.warn(`⚠️ FoundationModels ${operation} failed, using fallback:`, error.message);
...
`;
    ...
    return new Promise((resolve, reject) => {
      ...
      const timeout = setTimeout(() => {
        swift.kill('SIGTERM');
        reject(new Error('FoundationModels embedding timeout (15s)'));
      }, 15000);
...
guard model.availability == .available else {
    print("ERROR: FoundationModels model not available")
    exit(1)
}
...
`;
    ...
    return new Promise((resolve, reject) => {
      ...
      const timeout = setTimeout(() => {
        swift.kill('SIGTERM');
        reject(new Error('FoundationModels fact extraction timeout (10s)'));
      }, 10000);
      ...
      swift.on('close', (code) => {
        ...
        if (code !== 0) {
          reject(new Error(`FoundationModels Foundation Models failed: ${error}`));
          return;
        }
        ...
        console.log('✅ Extracted facts using FoundationModels Foundation Models');
  ...
  async addMemory(params) {
    try {
      ...
      console.log('🍎 Processing memory with FoundationModels...');

      // Extract facts using FoundationModels with timeout protection
  ...
  async testConnection() {
    const status = {
      ...
      message: '🍎 Native Node.js FoundationModels Memory System Online!',
    };
        ...
        tools: [
          {
            ...
            description: 'Test Native Node.js FoundationModels memory system connection',
            inputSchema: {
              type: 'object',
              properties: {},
            },
          ...
          {
            name: 'add_memory',
            description: 'Add new memory using native FoundationModels processing',
            inputSchema: {
              type: 'object',
              properties: {
                messages: {
          ...
          },
          {
            ...
            description: 'Search memories using native FoundationModels semantic understanding',
            inputSchema: {
              type: 'object',
              properties: {
                query: {
  ...
  async performHealthCheck() {
    ...
    try {
      ...
      console.warn('⚠️ FoundationModels degraded:', error.message);
    }
  ...
  async run() {
    ...
    console.error('🍎 Native Node.js FoundationModels Memory MCP Server running with self-healing');
