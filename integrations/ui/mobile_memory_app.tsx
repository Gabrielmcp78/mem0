...
export const MobileMemoryApp = () => {
  ...
  const loadMemories = async () => {
    setLoading(true);
    try {
      const response = await fetch('http://localhost:18765/api/v1/memories/');
  ...
  const handleSearch = async (query) => {
    ...
    setLoading(true);
    try {
      const response = await fetch('http://localhost:18765/api/v1/memories/filter', {
        method: 'POST',
  ...
  const handleDelete = async (memory) => {
    try {
      await fetch(`http://localhost:18765/api/v1/memories/${memory.id}`, {
        method: 'DELETE',
      });
